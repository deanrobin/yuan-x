...
public class QqNotifier implements Notifier {
    ...
    public boolean isEnabled() {
        return notifyConfig.getQq().isEnabled()
                && notifyConfig.getQq().getWebhookUrl() != null
                && !notifyConfig.getQq().getWebhookUrl().isBlank();
    }
    ...
    public void send(NotifyMessage message) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(notifyConfig.getQq().getWebhookUrl()))
            ...
            if (response.statusCode() == 200) {
                log.info("[QQ] Notified: {}", message.getTweetId());
            } else {
